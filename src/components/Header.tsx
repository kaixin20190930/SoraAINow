...
const navigation = {
    ...
    social: [
        {
            name: 'GitHub',
            href: 'https://github.com/kaixin20190930/SoraAINow',
            icon: (props) => (
...
export default function Header({
    ...
    return (
        <header className="sticky top-0 bg-[#000000] z-20 w-full">
            ...
            <LogoutModal
                ...
                redirectPath={`/${locale}/${page}`}
            />
            <nav className="mx-auto flex max-w-7xl items-center justify-between gap-x-6 p-4 lg:px-0"
                 aria-label="Global">
                <div className="flex lg:flex-1">
                    <a href={`/${locale}`} className="-m-1.5 p-1.5" onClick={() => setShowLoadingModal(true)}>
                        <Image className="h-20 w-20" src="/appicon.svg" alt="soraainow.com" width={48} height={48}/>
                    </a>
                ...
                <div className="hidden lg:flex lg:gap-x-24 md:text-2xl">
                    {navigation.topMenu.map((item) => (
                        <a
                            key={item.name}
                            href={`/${locale}${item.href}`}
                            onClick={() => setShowLoadingModal(true)}
                            className="text-sm font-semibold leading-10 text-white hover:text-blue-500"
                        >
                            {item.name}
                        </a>
                    ))}
                    ...
                    <Menu as="div" className="relative inline-block text-left">
                        <div>
                            <Menu.Button
                                ...
                                <GlobeAltIcon
                                    className="w-5 h-5 text-white"/>{locale == 'default' ? 'EN' : locale.toUpperCase()}
